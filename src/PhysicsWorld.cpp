	...
	int numCollisionObjects = rbWorld->getNumCollisionObjects();
	{
		...
		for (int i = 0; i < numCollisionObjects; i++)
		{
			...
			if (index >= 0)
			{
				...
				Ogre::Vector3 origin(btorigin->getX(), btorigin->getY(),
					btorigin->getZ());
				Ogre::Quaternion rotation(transform->getRotation().getW(),
				...
				double x = btorigin->getX();
				double y = btorigin->getY();
				double z = btorigin->getZ();
